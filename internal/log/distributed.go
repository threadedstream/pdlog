...
func (l *DistributedLog) WaitForLeader(timeout time.Duration) error {
	...
	for {
		...
		case <-ticker.C:
			if serverAddr, _ := l.raft.LeaderWithID(); serverAddr != "" {
				return nil
			}
		}
	}
...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	lStore, err := newLogStore(logDir, logConfig)
	if err != nil {
		return err
	}
	...
	if l.config.Raft.LeaseTimeout != 0 {
...
func (l *fsm) Restore(snapshot io.ReadCloser) error {
...
func (l *fsm) applyAppend(b []byte) any {
	...
	offset, err := l.log.Append(req.GetRecord())
	if err != nil {
		return err
	}
...
func newLogStore(dataDir string, config Config) (*logStore, error) {
	log, err := NewLog(dataDir, config)
	if err != nil {
		return nil, err
	}
...
func (l *logStore) GetLog(index uint64, out *raft.Log) error {
	...
	out.Data = in.GetValue()
	out.Index = in.GetOffset()
	out.Type = raft.LogType(in.GetType())
	out.Term = in.GetTerm()
	return nil
}
...
func (s *StreamLayer) Accept() (c net.Conn, err error) {
